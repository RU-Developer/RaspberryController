...
| SMD_ModuleManager_01 | void Init() | 필요한 모든 모듈 생성 및 초기화한다. | SAD_1 |
| SMD_ModuleManager_02 | vector<Module*>& GetModules() | 현재 사용중인 모든 모듈을 vector에 넣어 반환한다. | SAD_1 |
| SMD_ModuleManager_03 | void CreateModule(ModuleType type, int buttonNumber) | 모듈을 생성해서 관리한다. | SAD_1 |
